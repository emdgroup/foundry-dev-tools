...
class CompassClient(APIClient):
    ...
    def api_update_resource_roles(
        ...
        **kwargs,
        ...
        Note: Updates the role grants for a resource. Unlike OrganizationAndProjectService's updateResourceRoles endpoint, this endpoint uses the new `gk:all-without-mandatory` operation instead of the `gk:all` operation to disable inherited permissions. The disable inherited permissions statements generated by OrganizationAndProjectService's updateResourceRoles endpoint, on the resource (if any) will be removed if this endpoint is used. This endpoint cannot be used for projects or service project resources. To update roles on a project, use updateProject. This endpoint cannot be used for hidden resources. Throws the following errors: InsufficientPermissions if the user does not have "compass:change-resource-permission" on the resource AutosaveResourceOperationForbidden if resource is an autosave resource ForbiddenOperationOnHiddenResource if resource is a hidden resource ForbiddenOperationOnServiceProjectResource if resource is a service project or a service project resource
