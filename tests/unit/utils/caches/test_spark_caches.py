...
def test_add_get_delete(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    assert len(pc) == 0
...
def test_iterate(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    pc[to_dict("d1", "t11", "/d1")] = spark_session.createDataFrame(data=[[1, 2]], schema="a: int, b: int")
    pc[to_dict("d2", "t21", "/d2")] = spark_session.createDataFrame(data=[[1, 2]], schema="a: int, b: int")
    pc[to_dict("d3", "t31", "/d3")] = spark_session.createDataFrame(data=[[3, 4]], schema="c: int, d: int")
...
def test_key_not_exists_throws_key_error(test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    assert len(pc) == 0
    with pytest.raises(KeyError):
...
#
def test_unicode(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    data = spark_session.createDataFrame(data=[["\ud800foo", 2]], schema="a: string, b: string")
...
def test_reads_csv_files_format(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    df = spark_session.createDataFrame(data=[[1, 2]], schema="a: int, b: int")
    spark_schema = df.schema.jsonValue()
    dataset_directory = test_context_mock.config.cache_dir.joinpath("d1", "t1.csv")
    df.write.format("csv").option("header", "true").save(os.fspath(dataset_directory))
...
def test_supports_foundry_schema(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    df = spark_session.createDataFrame(data=[[1, 2]], schema="a: int, b: int")
...
def test_multiline_csv(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    dataset_directory = test_context_mock.config.cache_dir.joinpath("ds_multiline", "t1.csv")
...
0,"12345 asdf 1234 (asdf), Ba1234","Bawefawef Pharma International Ltd: Country:
 No Development Reported: Aram foo bar asdf: 16-Feb-2011",2020-01-28"""
    ...
    assert isinstance(
        from_cache["Test_Status_Current_Bar__CountryTerritory__Status__Indication__Date"].to_numpy()[0],
        str,
    )
...
def test_multiple_csvs_with_header(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
    dataset_directory = test_context_mock.config.cache_dir.joinpath("ds_multiline", "t1.csv")
...
Max,Mustermann"""
    ...
    assert_frame_equal(
        ...
        pd.DataFrame(data={"given_name": ["Max", "Hans"], "family_name": ["Mustermann", "Doe"]})
...
def test_cache_keeps_two_old_transactions(spark_session, test_context_mock):
    pc = DiskPersistenceBackedSparkCache(ctx=test_context_mock)
